...
import { BASE_SCORE, GUESS_PENALTY, HOTSPOT_PENALTY, MAX_GUESSES_PER_RUN, MEMORY_PENALTY, TIME_BUCKET_MS, TIME_BUCKET_PENALTY } from "@/convex/scoring";
...
function IndexInner() {
  ...
  if (waitingForBoot) return (<View style={styles.loadingContainer}><ActivityIndicator color={theme.accent} /><Text style={styles.loadingText}>Opening today’s archive…</Text></View>);
  if (session.episode === null) return (<View style={styles.loadingContainer}><ActivityIndicator color={theme.accent} /><Text style={styles.loadingText}>Preparing the first episode…</Text></View>);
  ...
  if (!currentScene) return (<View style={styles.loadingContainer}><ActivityIndicator color={theme.accent} /><Text style={styles.loadingText}>Generating today’s memories…</Text></View>);
  ...
  return (
    <View style={styles.root}>
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.content, { paddingTop: session.insets.top + 18, paddingBottom: session.insets.bottom + 28 }]} contentInsetAdjustmentBehavior="automatic">
        <View style={styles.hero}>
          ...
          <View style={styles.heroContent}>
            <View style={styles.brandRow}>
              <View style={styles.logoMark}><Ionicons name="eye" size={22} color={theme.inkOnAccent} /></View>
              ...
              {session.archiveCount > 0 && <Pressable style={styles.archiveBadge} href="/archive"><Ionicons name="archive-outline" size={11} color={theme.ink} /><Text style={styles.archiveBadgeText}>{session.archiveCount}</Text></Pressable>}
            </View>
        ...
        {isSolved && <View style={styles.nextActionsRow}><Pressable style={styles.nextActionButton} href="/archive"><Ionicons name="archive-outline" size={14} color={theme.ink} /><Text style={styles.nextActionText}>Archive</Text></Pressable><Pressable style={styles.nextActionButton} onPress={handleShareResult}><Ionicons name="share-outline" size={14} color={theme.ink} /><Text style={styles.nextActionText}>Share</Text></Pressable><Pressable style={styles.nextActionButton} onPress={() => setHistoryOpen(true)}><Ionicons name="list-outline" size={14} color={theme.ink} /><Text style={styles.nextActionText}>History</Text></Pressable><Pressable style={styles.nextActionButton}><Ionicons name="calendar-outline" size={14} color={theme.ink} /><Text style={styles.nextActionText}>Tomorrow</Text></Pressable></View>}
